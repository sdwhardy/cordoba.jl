...
result_mip=FileIO.load("C:\\Users\\shardy\\Documents\\julia\\times_series_input_large_files\\chandra\\result_Bewf_k5.jld2")
...
for (nw_i,nw) in sort(OrderedCollections.OrderedDict(result_mip["solution"]["nw"]), by=x->parse(Int64,x))
    ...
    if (issubset([nw_i],string.(year1_keys))) 
        ...
        #w_y1=w_y1+mn_data["nw"][nw_i]["gen"]["4"]["pmax"]
    ...
    #    w_y2=w_y2+mn_data["nw"][nw_i]["gen"]["4"]["pmax"]
    else
        for (g_i,g) in nw["gen"]
   ...
   #     w_y3=w_y3+mn_data["nw"][nw_i]["gen"]["4"]["pmax"]
    end 
end
...
println(string(wbenefits_y1/24)*" "*string(w_y1/12*100*8760/(24*argz["k"])*5*40)) 
println(string(wbenefits_y2/12)*" "*string(w_y2/12*100*8760/(24*argz["k"])*5*40)) 
println(string(wbenefits_y3/12)*" "*string(w_y3/12*100*8760/(24*argz["k"])*5*40))

#total Wnd
println(w_y1/12*100*8760/(24*argz["k"])*5*40) 
println(w_y2/12*100*8760/(24*argz["k"])*5*40) 
println(w_y3/12*100*8760/(24*argz["k"])*5*40)

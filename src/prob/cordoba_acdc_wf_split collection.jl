...
export cordoba_acdc_wf_split_collection
#**#
function cordoba_acdc_wf_split_collection(data::Dict{String,Any}, model_type::Type, solver; ref_extensions = [_PMACDC.add_ref_dcgrid!, _PMACDC.add_candidate_dcgrid!,  add_candidate_storage!, _PM.ref_add_on_off_va_bounds!, _PM.ref_add_ne_branch!], setting = s, kwargs...)
    s = setting
    return run_model_p1(data, model_type, post_cordoba_acdc_wf_split_collection; ref_extensions = [_PMACDC.add_ref_dcgrid!,_PMACDC.add_candidate_dcgrid!,  add_candidate_storage!, _PM.ref_add_on_off_va_bounds!, _PM.ref_add_ne_branch!], setting = s, kwargs...)
end


#**#
function post_cordoba_acdc_wf_split_collection(pm::_PM.AbstractPowerModel)
    ...
    vsp=[];vcp=[];vap=[];vbp=[];vgp=[];vdp=[];vacp=[]
        ...
        if (length(vsp)>1)
        sort!(vsp, by = x -> x[1])
        constraint_t0t1(vsp,pm);end

        if (length(vsp)>1)
        sort!(vbp, by = x -> x[1])
        constraint_t0t1(vbp,pm);end
        
        if (length(vsp)>1)
        sort!(vap, by = x -> x[1])
        constraint_t0t1(vap,pm);end
        
        if (length(vsp)>1)
        sort!(vcp, by = x -> x[1])
        constraint_t0t1(vcp,pm);end
        
        if (length(vsp)>1)
        sort!(vgp, by = x -> x[1])
        constraint_t0t1_wfz(vgp,pm);end
    ...
    #CONSTRAINTS: defined within PowerModels(ACDC) can directly be used, other constraints need to be defined in the according sections of the code: flexible_demand.jl
        for n in _PM.nw_ids(pm)
            ...
            ############################### collection circuit specific constraints #######################
            ############################### no crossings ###############################
            if ((haskey(pm.setting,"no_crossings") && pm.setting["no_crossings"]==true))
                constraint_no_crossings(pm; nw=n)
            end
            ############################### max strings per oss ########################
            if (haskey(pm.setting,"oss_nodes") && haskey(pm.setting,"max_num_strings_per_oss"))
                constraint_oss_connections(pm; nw=n)
            end
            ############################### max strings per oss ########################
            if (haskey(pm.setting,"turbine_nodes") && haskey(pm.setting,"max_num_of_branches_per_turbine"))
                constraint_turbine_connections(pm; nw=n)
            end
            ############################### max turbines per string ########################
            if (haskey(pm.setting,"max_turbines_per_string"))
                constraint_max_turbines_per_string(pm; nw=n)
            end
            ################################################################################
            if (haskey(pm.setting,"no_loops"))
                constraint_no_loops(pm; nw=n)
            end
            ######################################################################################
        end

export cordoba_mp_acdctnepopf_stoch

""
function cordoba_mp_acdctnepopf_stoch(data::Dict{String,Any}, model_type::Type, solver; ref_extensions = [_PMACDC.add_ref_dcgrid!, _PMACDC.add_candidate_dcgrid!, _PM.ref_add_on_off_va_bounds!, _PM.ref_add_ne_branch!], setting = s, kwargs...)
    s = setting
    return _PM.run_model(data, model_type, solver, post_cordoba_mp_acdctnepopf_stoch; ref_extensions = [_PMACDC.add_ref_dcgrid!, _PMACDC.add_candidate_dcgrid!, _PM.ref_add_on_off_va_bounds!, _PM.ref_add_ne_branch!], setting = s, kwargs...)
end
...
""
function post_cordoba_mp_acdctnepopf_stoch(pm::_PM.AbstractPowerModel)
...
if haskey(pm.setting, "compare_mode") && pm.setting["compare_mode"] == true
    objective_min_cost_opf(pm)
else
    objective_min_cost_acdc_cordoba(pm)
end

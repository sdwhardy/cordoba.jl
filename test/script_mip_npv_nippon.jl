...
function admm_4_AjAwAgAuAo_main(mn_data_mip, gurobi, s)
    results_set=[]
    eps=0.05#set max value of residual for convergence
    residual=Inf
    agents=["Ag","Au","Aob","Aw","Aj"]#Initilize agants
    ...
    #for i=1:10
        ...
        for a in agents
            s["agent"]=a
            println(a)
            result_mip = _CBD.acdc_tnep_convex_conv_strg_admm(mn_data_mip, _PM.DCPPowerModel, gurobi, multinetwork=true; setting=s)#try to print objective function

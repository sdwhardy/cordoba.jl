...
##################### Input parameters #################################
rt_ex=pwd()*"\\test\\data\\input\\DE_wf\\"#folder path
argz = Dict(
...
result_mip=FileIO.load("C:\\Users\\shardy\\Documents\\julia\\times_series_input_large_files\\chandra\\result_dewf_k5.jld2")
...
for (nw_i,nw) in sort(OrderedCollections.OrderedDict(result_mip["solution"]["nw"]), by=x->parse(Int64,x))
    ...
    if (issubset([nw_i],string.(year1_keys))) 
        ...
        wbenefits_y1=wbenefits_y1+nw["gen"]["4"]["pg"]*mn_data["nw"][nw_i]["gen"]["2"]["cost"][1]
        w_y1_ttl=w_y1_ttl+mn_data["nw"][nw_i]["gen"]["4"]["pmax"]
        w_y1=w_y1+nw["gen"]["4"]["pg"]
    elseif (issubset([nw_i],string.(year2_keys)))
        for (g_i,g) in nw["gen"]
            benefits_y2=benefits_y2+g["pg"]*mn_data["nw"][nw_i]["gen"][g_i]["cost"][1]
        end
        wbenefits_y2=wbenefits_y2+nw["gen"]["4"]["pg"]*mn_data["nw"][nw_i]["gen"]["2"]["cost"][1]
        w_y2_ttl=w_y2_ttl+mn_data["nw"][nw_i]["gen"]["4"]["pmax"]
        w_y2=w_y2+nw["gen"]["4"]["pg"]
    else
        for (g_i,g) in nw["gen"]
            benefits_y3=benefits_y3+g["pg"]*mn_data["nw"][nw_i]["gen"][g_i]["cost"][1]
        end
        wbenefits_y3=wbenefits_y3+nw["gen"]["4"]["pg"]*mn_data["nw"][nw_i]["gen"]["2"]["cost"][1]
        w_y3=w_y3+nw["gen"]["4"]["pg"]
        w_y3_ttl=w_y3_ttl+mn_data["nw"][nw_i]["gen"]["4"]["pmax"]
    end 
end
...
println(string(wbenefits_y1/24)*" "*string(w_y1/12*100*8760/(24*argz["k"])*5)) 
println(string(wbenefits_y2/12)*" "*string(w_y2/12*100*8760/(24*argz["k"])*5)) 
println(string(wbenefits_y3/12)*" "*string(w_y3/12*100*8760/(24*argz["k"])*5)) 

#wnd total
println(w_y1_ttl/12*100*8760/(24*argz["k"])*5*40) 
println(w_y2_ttl/12*100*8760/(24*argz["k"])*5*40) 
println(w_y3_ttl/12*100*8760/(24*argz["k"])*5*40) 

    ...
    ##################### Input parameters #################################
    rt_ex=pwd()*"\\test\\data\\input\\admm_UKBEDK\\"#folder path
    argz = Dict(
    "scenario_planning_horizon"=>30,
    "scenario_names"=>["EU17","EU18","EU19","EU20"],#Options: ["EU17","EU18","EU19","EU20","ST17","ST18","ST19","ST20","DG17","DG18","DG19","DG20"]
    "k"=>100,#number of representative days modelled (24 hours per day)
    "scenario_years"=>["2020"],#Options: ["2020","2030","2040"]
    "owpp_mva"=>[2000],#mva of wf in MVA
...
for (i,nw) in result_mip["solution"]["nw"]
    println(i)
    println(nw["gen"]["4"]["pg"])
    println(nw["gen"]["1"]["pg"]+nw["gen"]["5"]["pg"])
    println(nw["gen"]["2"]["pg"]+nw["gen"]["6"]["pg"])
    println(nw["gen"]["3"]["pg"]+nw["gen"]["7"]["pg"])
end

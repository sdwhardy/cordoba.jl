...
using Ipopt, Juniper, JuMP, Gurobi, Cbc, XLSX, DataFrames, Dates, CSV, FileIO, JLD2
...
#################### Scale cost data
_CBD.scale_cost_data_cordoba!(data_mip, scenario)
...
# Scale cost data
_CBD.scale_cost_data_cordoba!(data_conv, scenario)
_CBD.scale_cost_data_cordoba!(extradata, scenario)
...
println()
[println(string(mn_data_mip["nw"]["1"]["branchdc_ne"][k]["fbusdc"])*" - "*string(mn_data_mip["nw"]["1"]["branchdc_ne"][k]["tbusdc"])*" MVA: "*string(mn_data_mip["nw"]["1"]["branchdc_ne"][k]["rateA"]*100)) for (k,b) in result_mip["solution"]["nw"]["1"]["branchdc_ne"]]
